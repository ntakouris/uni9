...
int main(int argc, char *argv[])
{
    ...
    omp_set_dynamic(0);     // -- explicit disable dynamic teams
    int _len = AllPolylines.size();
    omp_set_num_threads(4); // -- N = 4 threads for below parallel region thread team
	struct timeval _s, _e;

	gettimeofday(&start, NULL);
	#pragma omp parallel private(polyline, simplified_polyline, _s, _e) shared(SimplifiedAllPolylines)
	{
		gettimeofday(&_s, NULL);

		#pragma omp for schedule(dynamic, 25) nowait // -- chunk size 25
		...
		double _total = ((_e.tv_sec - _s.tv_sec) * 1000000.0 + (_e.tv_usec - _s.tv_usec));
		int tid = omp_get_thread_num();
		printf("Thread time (%d) = %13.6f sec\n", tid, _total / 1000000.0);
	}

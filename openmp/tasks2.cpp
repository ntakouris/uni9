...
void RamerDouglasPeucker(const vector<Point> &pointList, double epsilon, vector<Point> &out)
{
	...
	if (dmax > epsilon)
	{
        ...
        // -- schedule this
        #pragma omp task shared(firstLine, recResults1)
        {
            RamerDouglasPeucker(firstLine, epsilon, recResults1);
        }

        #pragma omp task shared(lastLine, recResults2)
        {
            RamerDouglasPeucker(lastLine, epsilon, recResults2); // -- continue executin on same ctx
        }
        
        #pragma omp taskwait // -- wait for results
...
int main(int argc, char *argv[])
{
    ...
    omp_set_num_threads(4); // -- N = 4 threads for below parallel region thread team
    #pragma omp parallel for private(polyline, simplified_polyline) shared(SimplifiedAllPolylines) // -- any thread can get iteration and task creation work
